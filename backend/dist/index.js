...
// Connect to MongoDB
mongoose_1.default.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/mishti-chat')
...
app.post('/api/signup', async (req, res) => {
    try {
        ...
        await user.save();
        const token = jsonwebtoken_1.default.sign({ username: user.username, email: user.email }, SECRET, { expiresIn: '7d' });
...
app.post('/api/profile', authenticateToken, async (req, res) => {
    try {
        ...
        if (password && password.length >= 6) {
            update.password = await bcrypt_1.default.hash(password, SALT_ROUNDS);
        }
        if (profilePic) {
...
app.get('/api/rooms', async (req, res) => {
    try {
        ...
        let query = { isPrivate: false }; // Default to public rooms only
        if (username) {
...
const serializeRoomForSocket = (room) => {
    const roomData = room.toObject ? room.toObject() : room;
    return {
        ...roomData,
        isActive: roomData.participants && roomData.participants.length > 0
...
io.on('connection', (socket) => {
    ...
    socket.on('acceptFriendRequest', async (requestUsername) => {
        try {
            const user = await User.findOne({ username });
    ...
    socket.on('rejectFriendRequest', async (requestUsername) => {
        try {
            const user = await User.findOne({ username });
            if (!user)
                return;
    ...
    socket.on('addVideo', async ({ roomId, videoUrl }) => {
        try {
            if (!roomId || !videoUrl)
                return;
            const room = await Room.findOne({ id: roomId });
            if (!room || room.type !== 'Watch Together')
                return;
            const videoEntry = {
                url: videoUrl,
                score: 0,
    ...
    socket.on('voteVideo', async ({ roomId, videoIndex, voteType }) => {
        try {
            if (!roomId || videoIndex === undefined || !voteType)
                return;
            const room = await Room.findOne({ id: roomId });
            if (!room || room.type !== 'Watch Together')
                return;
    ...
    socket.on('removeVideoFromQueue', async ({ roomId, videoIndex }) => {
        try {
            if (!roomId || videoIndex === undefined)
                return;
            const room = await Room.findOne({ id: roomId });
            if (!room || !Array.isArray(room.videoQueue) || room.videoQueue.length === 0)
                return;
            if (videoIndex < 0 || videoIndex >= room.videoQueue.length)
                return;
